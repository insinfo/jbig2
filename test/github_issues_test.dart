  ...
  group('GithubIssuesTest', () {
    test('issue21', () {
      ...
      if (!globalsFile.existsSync()) {
        fail('Globals file not found: $globalsPath');
      }
      if (!imageFile.existsSync()) fail('Image file not found: $imagePath');

      final globalsRar =
          RandomAccessReadBuffer.fromBytes(globalsFile.readAsBytesSync());
      final globalsDoc = JBIG2Document(globalsRar);

      final globals = globalsDoc.globalSegments;

      final imageRar =
          RandomAccessReadBuffer.fromBytes(imageFile.readAsBytesSync());
